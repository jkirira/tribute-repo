<?php
	header('Content-Type: text/html; charset=utf-8');
?>


<!DOCTYPE html>

<head>
    <title>
        Fan Tribute
    </title>
</head>

<body>

    <header>
        <nav>
            
            <ol>
                <li><a href="#bio">Bio</a></li>
                <li><a href="#appearances">Appearances</a></li>
                <li><a href="#trophies">Trophies</a></li>
            </ol>
            
            <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 458.624 458.624" style="enable-background:new 0 0 458.624 458.624;" xml:space="preserve" width="50" height="50">
                <g>
                    <g>
                        <path d="M339.588,314.529c-14.215,0-27.456,4.133-38.621,11.239l-112.682-78.67c1.809-6.315,2.798-12.976,2.798-19.871
                        c0-6.896-0.989-13.557-2.798-19.871l109.64-76.547c11.764,8.356,26.133,13.286,41.662,13.286c39.79,0,72.047-32.257,72.047-72.047
                        C411.634,32.258,379.378,0,339.588,0c-39.79,0-72.047,32.257-72.047,72.047c0,5.255,0.578,10.373,1.646,15.308l-112.424,78.491
                        c-10.974-6.759-23.892-10.666-37.727-10.666c-39.79,0-72.047,32.257-72.047,72.047s32.256,72.047,72.047,72.047
                        c13.834,0,26.753-3.907,37.727-10.666l113.292,79.097c-1.629,6.017-2.514,12.34-2.514,18.872c0,39.79,32.257,72.047,72.047,72.047
                        c39.79,0,72.047-32.257,72.047-72.047C411.635,346.787,379.378,314.529,339.588,314.529z"/>
                    </g>
                </g>
            </svg>

            <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 486.805 486.805" style="enable-background:new 0 0 486.805 486.805;" xml:space="preserve" width="50" height="50">
                <g>
                    <path d="M485.9,241.402l-26.8-167c-5.2-41.9-34.5-66-80.4-66H243.6h-6.8h-65.9c-19.2,0-36.9,8.3-49.4,21.6
                    c-4.5-5.5-11.4-8.9-19-8.9H24.7c-13.6,0-24.7,11.1-24.7,24.7v228.4c0,13.6,11.1,24.7,24.7,24.7h77.9c9,0,17-4.9,21.3-12.2l2.9,0.7
                    c4.4,1.3,80.8,25,80.8,90.7v84.9c0,5.2,3.4,9.9,8.4,11.4c0.9,0.3,12.9,4,28.3,4c13.3,0,29.1-2.7,42.5-12.6
                    c18.4-13.5,27.7-36.5,27.7-68.4v-75.8h32.4h56.9h15.8c15.6,0,31-5.8,43.3-16.2C480.9,290.002,489.8,265.402,485.9,241.402z
                    M103.2,274.302c0,0.4-0.3,0.7-0.7,0.7H24.7c-0.4,0-0.7-0.3-0.7-0.7v-228.4c0-0.4,0.3-0.7,0.7-0.7h77.9c0.4,0,0.7,0.3,0.7,0.7
                    v228.4H103.2z M447.3,287.202c-7.9,6.8-17.8,10.5-27.7,10.5h-15.8h-56.9h-44.5c-6.6,0-12,5.4-12,12v87.8c0,23.8-6,40.3-17.8,49
                    c-13,9.6-30.8,8.6-41.1,7v-75.3c0-35.6-17.3-66.7-49.9-89.9c-23.9-16.9-47.6-23.7-48.6-24c-0.2,0-0.3-0.1-0.5-0.1l-5.2-1.2v-191.5
                    c0.2-0.7,0.4-1.4,0.5-2.1c3.3-21,21.8-36.9,43.1-36.9h65.9h6.8h135.1c33.9,0,52.9,15.2,56.6,45.1c0,0.2,0,0.3,0.1,0.4l26.9,167.1
                    C464.8,261.002,459,277.102,447.3,287.202z"/>
                </g>
            </svg>

            <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 490.2 490.2" style="enable-background:new 0 0 490.2 490.2;" xml:space="preserve" width="50" height="50">
                <g>
                    <path d="M490.1,234.464c0-37.6-30.6-68.3-68.3-68.3H316.2v-76.1c0-32.2-9.4-55.4-28-69c-29.4-21.6-69.8-9.3-71.5-8.7
                    c-5.1,1.6-8.6,6.3-8.6,11.7v85.5c0,67.3-78.1,90.2-81.4,91.2c-1,0.3-1.9,0.6-2.7,1.1c-5.8-11.6-17.8-19.6-31.6-19.6H35.2
                    c-19.4,0-35.2,15.8-35.2,35.2v207.9c0,19.4,15.8,35.2,35.2,35.2h33.1c6.8,0,12.3-5.5,12.3-12.3s-5.6-12.2-12.3-12.2H35.2
                    c-5.9,0-10.7-4.8-10.7-10.7v-207.9c0-5.9,4.8-10.7,10.7-10.7h57.3c5.9,0,10.7,4.8,10.7,10.7v196.1c0,37.6,30.6,68.3,68.3,68.3
                    h209.2c46.4,0,75.9-24.3,81.1-66.3l28.2-177c0.1-0.6,0.2-1.3,0.2-1.9v-2.2H490.1z M465.6,235.564l-28.1,176.5
                    c-3.7,30-22.8,45.3-56.8,45.3h-7.8H171.5c-24.1,0-43.8-19.6-43.8-43.8v-189.1c1.8,0.4,3.7,0.3,5.6-0.2
                    c4.1-1.1,99.3-28.7,99.3-114.8v-75.6c10.4-1.6,28.2-2.5,41.1,6.9c11.9,8.8,18,25.3,18,49.3v88.4c0,6.8,5.5,12.3,12.3,12.3h117.9
                    c24.1,0,43.8,19.6,43.8,43.8v1H465.6z"/>
                </g>
            </svg>

            <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 490.4 490.4" style="enable-background:new 0 0 490.4 490.4;" xml:space="preserve" width="50" height="50">
                <g>
                    <g>
                    <path d="M222.5,453.7c6.1,6.1,14.3,9.5,22.9,9.5c8.5,0,16.9-3.5,22.9-9.5L448,274c27.3-27.3,42.3-63.6,42.4-102.1
                    c0-38.6-15-74.9-42.3-102.2S384.6,27.4,346,27.4c-37.9,0-73.6,14.5-100.7,40.9c-27.2-26.5-63-41.1-101-41.1
                    c-38.5,0-74.7,15-102,42.2C15,96.7,0,133,0,171.6c0,38.5,15.1,74.8,42.4,102.1L222.5,453.7z M59.7,86.8
                    c22.6-22.6,52.7-35.1,84.7-35.1s62.2,12.5,84.9,35.2l7.4,7.4c2.3,2.3,5.4,3.6,8.7,3.6l0,0c3.2,0,6.4-1.3,8.7-3.6l7.2-7.2
                    c22.7-22.7,52.8-35.2,84.9-35.2c32,0,62.1,12.5,84.7,35.1c22.7,22.7,35.1,52.8,35.1,84.8s-12.5,62.1-35.2,84.8L251,436.4
                    c-2.9,2.9-8.2,2.9-11.2,0l-180-180c-22.7-22.7-35.2-52.8-35.2-84.8C24.6,139.6,37.1,109.5,59.7,86.8z"/>
                    </g>
                </g>
            </svg>

        </nav>
    </header>


    <main>
        <canvas id="myCanvas" width="300" height="150" style="border:1px solid #d3d3d3;">
        Your browser does not support the HTML5 canvas tag.
        </canvas>


        <script>
            var c = document.getElementById("myCanvas");
            var outer_circle = c.getContext("2d");
            var inner_circle = c.getContext("2d");

            outer_circle.beginPath();
            outer_circle.arc(100, 75, 50, 0, 2 * Math.PI);
            outer_circle.fillStyle = "red";
            outer_circle.fill();

            inner_circle.beginPath();
            inner_circle.arc(100, 75, 27, 0, 2 * Math.PI);
            inner_circle.fillStyle = "yellow";
            inner_circle.fill();

            outer_circle.stroke();
            inner_circle.stroke();
        </script> 

        <h2>Wayne Rooney</h2>
        <article id="bio">
            <h3>Bio</h3>
            <figure>
                <img src="assets/img/Rooney_1.jpg" alt="Wayne Rooney at Manchester United">
                <figcaption>Wayne Rooney at Manchester United in 2016.</figcaption>
              </figure> 
            <p>Full name &nbsp Wayne Mark Rooney</p>
            <p>Date of birth &nbsp 24 October 1985 (age 36)</p>
            <p>Place of birth &nbsp Liverpool, England</p>
            <p>Height &nbsp 5 ft 9 in (1.76 m)</p>
            <p>Position(s) &nbsp Forward, midfielder</p>
        </article>

        <section id="appearances">
            <h3>Appearances</h3>
            <table>
                <tr>
                    <th>Years</th>
                    <th>Team</th>
                    <th>No. of Appearances</th>
                </tr>
                <tr>
                    <td>2002–2004</td>
                    <td>Everton</td>
                    <td>67</td>
                </tr>
                <tr>
                    <td>2004–2017</td>
                    <td>Manchester United</td>
                    <td>393</td>
                </tr>
                <tr>
                    <td>2017–2018</td>
                    <td>Everton</td>
                    <td>31</td>
                </tr>
                <tr>
                    <td>2018–2020</td>
                    <td>D.C. United</td>
                    <td>48</td>
                </tr>
                <tr>
                    <td>2020–2021</td>
                    <td>Derby County</td>
                    <td>30</td>
                </tr>
            </table>
        </section>

        <section id="trophies">
            <h3>Trophies</h3>
            <details>
                <summary>Premier League (5)</summary>
                2006–07, 2007–08, 2008–09, 2010–11, 2012–13
            </details>
            
            <details>
                <summary>FA Cup (1)</summary>
                2015–16
            </details>

            <details>
                <summary>EFL Cup (3)</summary>
                2005–06, 2009–10, 2016–17
            </details>

            <details>
                <summary>FA Community Shield (4)</summary>
                2007, 2010, 2011, 2016
            </details>

            <details>
                <summary>UEFA Champions League (1)</summary>
                2007–08
            </details>

            <details>
                <summary>UEFA Europa League (1)</summary>
                2016–17
            </details>
            
            <details>
                <summary>FIFA Club World Cup (1)</summary>
                2008
            </details>
            
        </section>

	<section>
		<h3>Goal of the Season 2010/11</h3>
		<p>The Premier League Goal of the Season is an annual football award for the player to has deemed to have scored the best goal in the Premier League the preceding season. </p>
		<p>He won the Goal of the Season award for his February 2011 bicycle kick against city rivals Manchester City.</p>
		<p>Watch the goal here: </p>
		<iframe src="https://player.vimeo.com/video/128691257?h=6f71819da6" width="640" height="360" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe>
		<p><a href="https://vimeo.com/128691257">Wayne Rooney goal vs Manchester City (12.02.2011)</a> from <a href="https://vimeo.com/deanrayper">DeanRayper</a> on <a href="https://vimeo.com">Vimeo</a>.</p>
	</section>

    </main>

    <footer>
        <p>Author: James Kirira</p>
        <p>Email: jkirira@cytonn.com</p>
        (cc) Copied right.
    </footer>
</body>

</html>
